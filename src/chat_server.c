...
int main()
{
    ...
    while( 1 )
    {
        ...
        {
            if( !FD_ISSET(currentSocket, &reads) ) continue;
            ...
            else
            {
                ...
                for( int socket = 1; socket <= maxSocket; ++socket)
                {
                    if( FD_ISSET(socket, &master) )
                    {
                        if( socket == socketListen || socket == currentSocket ) continue;
                        send(socket, read, bytesReceived, 0);
                    }
                }
            }
        }
